...
public class BookReceiptDAL extends  DataAccessHelper{
        ...
        try {
            ...
            if(rs!=null)
                while(rs.next())
                {
                    ...
                    bookstock.setTotalReceipt(rs.getInt("TONGTIEN"));
                    temp.add(bookstock);
                }
            getClose();
        } catch (Exception e) {
    ...
    //Tính tổng chi phí do việc nhập sách
    public float TotalExpense(String FromDate, String ToDate)
    {
        String SQL = "SP_TOTAL_EXPENSE '" + FromDate + "', '" + ToDate + "'";
        try {
            getConnect();
            Statement st = conn.createStatement();
            ResultSet rs =st.executeQuery(SQL);
