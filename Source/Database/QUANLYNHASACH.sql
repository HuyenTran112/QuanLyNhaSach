...
GO
INSERT INTO KHUYENMAI VALUES(1, N'Khuyến mãi cuối năm',0.05,'20000', '30-10-2018' , '30-11-2018')
=======
>>>>>>> 421aeaefbd9b217cc7faff1ce273634205ea9fdd
...
BEGIN
	...
	IF(@SOHD_MAX IS NULL)
	BEGIN
		INSERT INTO HOADON VALUES(1, @NGAYHD_, @MAKH, @MANV, @VAT, NULL, NULL, NULL)
	END
	ELSE
	BEGIN
		WHILE(@I <= @SOHD_MAX + 1)
		BEGIN
			...
			BEGIN
				INSERT INTO HOADON VALUES(@I, @NGAYHD_, @MAKH, @MANV, @VAT, NULL, NULL, NULL)
				BREAK
			END
			SET @I += 1
		END
...
AS
BEGIN
	DECLARE @SOHD INT, @MASACH INT, @SOLUONG INT, @TONGSACHTON INT, @TRIGIA MONEY, @HESOKM FLOAT, @MAKM INT, @MAKH INT, @DOANHSO FLOAT, @SOLUONGBO INT, @SOHDBO INT, @MASACHBO INT
...
=======
ALTER TRIGGER TRG_INSUP_THANHTIEN_CTTT ON CTHD
FOR INSERT, UPDATE
AS
BEGIN
...
BEGIN
<<<<<<< HEAD
DECLARE @SOHD INT, @MASACH INT, @SOLUONG INT, @TONGSACHTON INT, @TRIGIA MONEY, @HESOKM FLOAT, @MAKM INT, @THANHTIEN MONEY, @MAKH INT, @DOANHSO FLOAT
	...
	SELECT  @THANHTIEN = (SELECT GIABAN FROM SACH WHERE MASACH = @MASACH) * @SOLUONG
	FROM deleted
...
AS
BEGIN
	INSERT INTO CTHD VALUES(@SOHD, @MASACH, @SOLUONG, NULL)
END
GO
--CẬP NHẬT CTHD
...
BEGIN
	...
	IF(@NGAYHD = '')
		SELECT * FROM CTHD
	ELSE
		SELECT CTHD.SOHD, MASACH, SOLUONG, THANHTIEN 
		FROM HOADON, CTHD
		WHERE HOADON.SOHD = CTHD.SOHD AND NGAYHD = @NGAYHD_
END

SELECT * FROM CTHD
SELECT * FROM HOADON

EXEC SP_SEARCHBILLINFO ''
=======
	DECLARE @SOHD INT, @MASACH INT, @SOLUONG INT, @GIA MONEY , @THANHTIEN MONEY, @TONGTIEN MONEY
	SELECT @SOHD=SOHD ,@MASACH=MASACH,@SOLUONG=SOLUONG
	FROM inserted
	SELECT @GIA=GIABAN
	FROM SACH
